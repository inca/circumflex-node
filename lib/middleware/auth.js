  ...
  return function auth(req, res, next) {
    ...
    req.logout = function(cb) {
      ...
      if (persistence) {
        ...
        if (req.session.authPersistenceToken)
          persistence.dropToken(user, req.session.authPersistenceToken, cb);
        else cb();
      } else cb();
    };
    ...
    req.tryPersistentLogin = function(cb) {
      ...
      var cookieValue = req.cookies[persistence.cookie.name];
      ...
      conf.auth.findUserById(userId, function(err, user) {
        ...
        persistence.hasToken(user, token, function(err, owns) {
          ...
          req.login(user, function(err) {
            if (err) return cb(err);
            // Drop this token
            persistence.dropToken(user, token, function(err) {
              if (err) return cb(err);
              // Refresh the persistence
              req.persistLogin(user, function(err) {
                if (err) return cb(err);
                // And finally try session login to populate the user
