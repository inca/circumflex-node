...
var Request = module.exports = exports = {
  ...
  getMoment: function(name, defaultValue) {
    var d = moment.utc(this.getString(name), 'YYYY-MM-DD');
    return d.isValid() ? d : moment.utc(defaultValue, 'YYYY-MM-DD');
  },
  ...
  getMoments: function(name, defaultValue) {
    ...
    if (Array.isArray(value))
      return flatMap(value, function(v) {
        var mom = moment.utc(v, 'YYYY-MM-DD');
        return mom.isValid() ? mom : null;
      });
    // Single value
    var mom = moment.utc(value, 'YYYY-MM-DD');
  ...
  getDate: function(name, defaultValue) {
    ...
    var mom = moment.utc(value, 'YYYY-MM-DD');
    return mom.isValid() ? mom.toDate() : defaultValue;
  },
  ...
  /**
   * Returns a parameter captured by the route from `req.params[name]`,
   * stripping it from leading dots and slashes (for security reasons).
   *
   * @param name -- route parameter index or name
   * @param defaultValue -- optional default value
   */
  getPath: function(name, defaultValue) {
    var param = this.getParam(name, defaultValue);
